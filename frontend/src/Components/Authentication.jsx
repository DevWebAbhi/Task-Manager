...
import { configureTokenAxios } from "../axiosInstance";
const Authentication = () => {
  ...
  const handleSignupRequest = useRef();
  ...
  // This useffect check weather token is stored in localstorage if stored navigate to /tasks
  ...
  useEffect(() => {
    ...
    return ()=>{
      clearInterval(handleSignupRequest.current);
    }
  }, []);

  //when person logs out it make sures all of stored credential should wipe out
    ...
    if (!selector.login) {
      ...
      try {
        ...
        if (authenticate.data.message == "invalid email") {
          ...
          handleSignupRequest.current=setInterval(async ()=>{
            try {
              const authenticate = await axios.post(
                `${REACT_APP_BACKEND_URL}user/login`,
                { email: selector.email, password: selector.password }
              );
              if(authenticate.data.message=='unauthorized access' || authenticate.data.message=='internal server error'){
                toast({
                  title: `Something went wrong`,
                  position: "top-right",
                  isClosable: true,
                });
                clearInterval(handleSignupRequest);
              }else if(authenticate.data.message=="process"){
                toast({
                  title: `Check your email and verify`,
                  position: "top-right",
                  isClosable: true,
              ...
              }else if(authenticate.data.message=="successfull"){
                toast({
                  title: `Check your email and verify`,
                  position: "top-right",
                  isClosable: true,
          ...
          // This makes sure the localstorage (its async process take time to update) changes by refreshing the page.
  ...
  return (
    ...
    >
      ...
      >
        <FormControl>
          <FormLabel
            display={!selector.login ? "flex" : "none"}
            fontFamily="Poppins"
            marginBottom="0.5rem"
          >
            UseName
          ...
          />
          <FormLabel fontFamily="Poppins" marginBottom="0.5rem">
            Email
          </FormLabel>
          <Input
            fontFamily="Poppins"
          ...
          <Flex
            ...
            marginTop="0.7rem"
          >
            <Text fontFamily="Poppins" fontSize="large">
